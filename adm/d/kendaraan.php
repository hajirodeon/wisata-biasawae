...
if ($_POST['btnSMP'])
	...
	//nek null
	if ((empty($e_nama)) OR (empty($e_tempat)) OR (empty($e_harga)))
		{
		//re-direct
		...
		if ($s == "edit")
			{
			mysqli_query($koneksi, "UPDATE tempat_kendaraan SET nama = '$e_nama', ".
							"harga = '$e_harga', ".
							"tempat_kd = '$e_tempat', ".
		...
		if ($s == "baru")
			{
			//cek
			$qcc = mysqli_query($koneksi, "SELECT * FROM tempat_kendaraan ".
									"WHERE nama = '$e_nama' ".
			...
			if ($tcc != 0)
				...
				{
				mysqli_query($koneksi, "INSERT INTO tempat_kendaraan(kd, tempat_kd, tempat_nama, ".
								"nama, harga, postdate) VALUES ".
								"('$kd', '$e_tempat', '$e_tempat_nama', ".
								"'$e_nama', '$e_harga', '$today')");
...
if ($_POST['btnHPS'])
	...
	for ($i=1; $i<=$jml;$i++)
		...
		//del
		mysqli_query($koneksi, "DELETE FROM tempat_kendaraan ".
						"WHERE kd = '$kd'");
		}
...
if (($s == "baru") OR ($s == "edit"))
	...
	$qx = mysqli_query($koneksi, "SELECT * FROM tempat_kendaraan ".
						"WHERE kd = '$kdx'");
	$rowx = mysqli_fetch_assoc($qx);
	...
	Tempat Wisata : 
	<br>
	<select name="e_tempat" class="btn-success" required>
	...
	Biaya Sewa (Per Hari) : 
	<br>
	Rp. <input name="e_harga" type="text" value="'.$e_harga.'" size="10" class="btn-success">,-
	</p>
	...
	if (empty($kunci))
		{
		$sqlcount = "SELECT * FROM tempat_kendaraan ".
	...
	<td><strong><font color="'.$warnatext.'">NAMA KENDARAAN</font></strong></td>
	<td width="200"><strong><font color="'.$warnatext.'">BIAYA SEWA</font></strong></td>
	</tr>
	</thead>
	<tbody>';
