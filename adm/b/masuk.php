...
if (empty($kunci))
	...
	$sqlcount = "SELECT * FROM orderan ".
					"WHERE bayar_postdate IS NOT NULL ".
...
<td width="200"><strong><font color="'.$warnatext.'">POSTDATE BAYAR</font></strong></td>
...
if ($count != 0)
	...
	do 
		...
		$i_bayar_postdate = balikin($data['bayar_postdate']);

		
		echo "<tr valign=\"top\" bgcolor=\"$warna\" onmouseover=\"this.bgColor='$warnaover';\" onmouseout=\"this.bgColor='$warna';\">";
		echo '<td>'.$i_bayar_postdate.'</td>
		<td>'.$i_tempat.'</td>
		<td>'.$i_pintu_masuk.'</td>
		<td>'.$i_kodebooking.'</td>
